...
public class GeekbenchCpu implements Crawler<Cpu> {
	...
	public Cpu crawl() {
		...
		String brand = Cpu.parseBrand(name);
		String model = Cpu.parseModel(name);
		...
		return new Cpu(brand, model, cores, threads, frequency, maxFrequency, scoreSingleCore, scoreMultiCore);
	}
